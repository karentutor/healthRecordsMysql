...
class SingleRecord extends Component {
    ...
    likeToggle = () => {
        ...
        const recordId = this.state.record._id;
    ...
    renderRecord = record => {
        const posterId = record.postedBy ? `/user/${record.postedBy._id}` : '';
        const posterName = record.postedBy ? record.postedBy.name : ' Unknown';
        ...
        return (
            <div className="card-body">
                ...
                <div className="d-inline-block">
                    <Link to={`/findrecords`} className="btn btn-raised btn-primary btn-sm mr-5">
                        Back to records
                    </Link>

                    {isAuthenticated().user && isAuthenticated().user._id === record.postedBy._id && (
                        <>
                            <Link to={`/record/edit/${record._id}`} className="btn btn-raised btn-warning btn-sm mr-5">
                                Update Record
                            </Link>
